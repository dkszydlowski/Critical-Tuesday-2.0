...
ODLMAR = function(nl,delta,x.full,T.full,title) {
  ...
  plot(T.ar,yhat,type='l',lwd=1,col='deepskyblue',xlab='DOY',ylab='Y and Yhat',main=title)
  points(T.ar,Y,type='p',pch=19,cex=0.5,col='darkblue')
  plot(T.ar,yhat-Y,type='l',lwd=1,col='red',xlab='DOY',ylab='one-step error')
  plot(T.ar,B.ests[1,],type='l',lwd=1,col='forestgreen',xlab='DOY',ylab='intercept')
  plot(T.ar,vupdate,type='l',lwd=1,col='blue',xlab='DOY',ylab='updated error var')
  ...
  yrange = range(lamda,0,1)
  plot(T.ar,lamda,type='l',lwd=1,col='blue',ylim=yrange,
       xlab='DOY',ylab='Eigenvalue ',main=title)
  abline(h=1,lty=2)
  
  # Create output list and return it
  Yyhat = matrix(c(T.ar,Y,yhat,vupdate),nr=(nobs-nl),nc=4)
  
  outlist = list(Yyhat,lamda,B.ests,B.sd,vupdate)
  return(outlist)
