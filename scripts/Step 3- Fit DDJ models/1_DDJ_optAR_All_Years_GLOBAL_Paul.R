...
#load(file="./scripts/Langevin/HF Langevin/DLMresult_HYLB_Paul_ALL_Chl.Rdata")
Fname = c(paste('./results/DDJ results Paul ARIMA-corrected data.Rdata', sep = ""))

# read in dataframe from diagnostic step
DDJ_prepped = read.csv("./data/formatted data/HF data/ARIMA Paul stacked data for global DDJ 098 NOISY THINNED.csv")
...
write.csv(all, paste("./results/DDJ results Paul ARIMA-correced", ".csv", sep = ""), row.names = FALSE)
write.csv(all.deriv, paste("./results/DDJ results deriv Paul ARIMA-correced", ".csv", sep = ""), row.names = FALSE)
